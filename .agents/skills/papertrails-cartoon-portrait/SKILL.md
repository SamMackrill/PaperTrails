...
4. Generate or edit one scientist at a time. Treat the source portrait as an identity reference, not as pixels to colourise. Ask for a newly drawn editorial caricature in the house style.
5. Inspect the result at full size, 92 px, and 42 px in a circle. For review, always show a three-way comparison: source photo/portrait, current cartoon, and proposed redraw. Use `pwsh -File .agents/skills/papertrails-cartoon-portrait/scripts/build-review-sheet.ps1 -ScientistIds <id-a>,<id-b> -CandidateDirectory <directory> -OutputPath <review.png>` when the conventional asset names apply. The proposed column deliberately composites the cutout over dark and light colours so transparency defects are visible. Reject it if the identity, silhouette, expression, or cutout quality disappears at either UI size.
6. Save the candidate to a staging filename and run `pwsh -File .agents/skills/papertrails-cartoon-portrait/scripts/validate-cartoon-asset.ps1 -Path <candidate.png>`. Never infer transparency from a checkerboard shown by an image tool or preview.
7. If an otherwise approved generation has a baked pale checkerboard or the wrong dimensions, run `pwsh -File .agents/skills/papertrails-cartoon-portrait/scripts/finish-cartoon-asset.ps1 -InputPath <generated.png> -OutputPath <candidate.png>`. This deterministic step removes only pale neutral background connected to the canvas edge, retains enclosed white portrait details, writes real alpha, and normalises the canvas to 1024×1024. Do not use it on scenic, coloured, or uncertain backgrounds.
8. Run the validator again on the finished candidate. Reject it if validation still fails or visual inspection reveals halos, holes, clipped outlines, or lost pale details. A depicted or baked checkerboard is an opaque background, not transparency, and neither a generation nor a finishing command may bypass the validation gate.
9. After both visual and file validation pass, preserve an existing entry's `cartoon` path unless it is the shared default. A new portrait must never reuse `images/cartoons/default.png`. When an existing entry points to that default and a trustworthy likeness is available, assign a unique scientist-specific path, update only that entry, and preserve every other reference to the shared default. For a new path containing both names, use `images/cartoons/<surname>_<givenname>.png`; keep surname particles together, and use a surname-only filename when unambiguous. Do not replace or retarget an existing portrait until the replacement has been inspected.
...
Constraints: transparent background; 1024×1024; preserve identity and period details; no text; no watermark
Avoid: colourised-photo realism; painterly rendering; airbrushing; yellow or sepia skin cast; generic science clip-art; full body; elaborate background
```
